...
Students::Students(int Id, string name, string password)
{
	...
	{
		_ComputerRoom.push_back(computer);
	}
	ifs.close();
}
...
void Students::ApplyReservation()
{
	...
	{
		if (it->second["日期"] == to_string(date) && it->second["时间段"] == to_string(time) && it->second["学号"] == to_string(this->_UserId))
		{
	...
	cout << "请选择机房：" << endl;
	for (unsigned int i = 0; i < _ComputerRoom.size(); ++i)
	{
		cout << i + 1 << "号机房容量为：" << _ComputerRoom[i] << endl;
	}
	while (true)
	{
		cin >> room;
...
//保存机房信息
void Students::SaveComputerRoom()
{
	ofstream ofs;
	ofs.open(COMPUTERROOM_FILE, ios::trunc);
	for (unsigned int i = 0; i < _ComputerRoom.size(); ++i)
	{
		ofs << i + 1 << "   " << _ComputerRoom[i] << endl;
	}
	ofs.close();
}
